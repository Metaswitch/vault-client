    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<RevokeLeaseResponse, ApiError>
    {
        ...
        request.headers_mut().append(
            ...
            match header::IntoHeaderValue(param_x_vault_token.clone()).try_into() {
                Ok(header) => header,
                Err(e) => {
                    return Err(ApiError(format!(
                        "Invalid header x_vault_token - {}", e)));
                },
            });
        ...
        context: &C) -> Result<GenerateCertResponse, ApiError>
    {
        ...
        let mut uri = format!(
            "{}/v1/{mount}/issue/{name}",
            self.base_path
            ,mount=utf8_percent_encode(&param_mount.to_string(), ID_ENCODE_SET)
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::GenerateCertificateResponse>(body)?;
                Ok(GenerateCertResponse::Success
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<ReadCertResponse, ApiError>
    {
        ...
        let mut uri = format!(
            ...
            self.base_path
            ,mount=utf8_percent_encode(&param_mount.to_string(), ID_ENCODE_SET)
            ,serial=utf8_percent_encode(&param_serial.to_string(), ID_ENCODE_SET)
        );
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::CertificateResponse>(body)?;
                Ok(ReadCertResponse::Success
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<CreateOrphanTokenResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::AuthResponse>(body)?;
                Ok(CreateOrphanTokenResponse::Success
                    (body)
                )
            }
            code => {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::AuthResponse>(body)?;
                Ok(CreateTokenResponse::Success
                    (body)
                )
            }
            code => {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::AuthResponse>(body)?;
                Ok(LogInWithTLSCertificateResponse::Success
                    (body)
                )
            }
            code => {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::AuthResponse>(body)?;
                Ok(RenewOwnTokenResponse::Success
                    (body)
                )
            }
            code => {
